...
class ListConfig:
    ...
    def __create_defult_config(self):
        ...
        self.parser.add_section(section)
        self.parser.set(section, 'shopping', os.path.join(resources,
                                                          'shopping_ua.xml'))
        self.parser.set(section, 'travel', os.path.join(resources,
                                                        'travel_ua.xml'))
        self.__update_file()
...
class Products:
    ...
    def _make_sure(self, filepath):
        '''if the given filepath is not a file,
        create one with the XML header.'''
    ...
    def _get_data(self):
        ...
        for depart in root:
            products = []
            for prod in depart:
                products.append({'name': prod.attrib['name'],
                                 'chk': prod.attrib['inlist']})
            departs.append({'name': depart.attrib['name'],
                            'products': products})
        return departs
    ...
    def save_data(self):
        ...
        for dep in self._departs:
            ...
            for prod in dep['products']:
                ET.SubElement(d, 'product',
                              {'inlist': prod['chk'], 'name': prod['name']})
    ...
    def get_all_products(self):
        return [unicode(prod['name'])
                for dep in self._departs for prod in dep['products']]

    def get_checked(self):
        # return a list of names that are checked
        return [unicode(x['name'])
                for x in self._products() if self.is_product_checked(x)]

    def get_unchecked(self):
        # return a list of names that are NOT checked
        return [unicode(x['name'])
                for x in self._products() if not self.is_product_checked(x)]
    ...
    def add_depart(self, name):
        if name in [x['name'] for x in self._departs]:
            msg = u'A department with name %s has been in the list' % name
            self.last_msg = msg
            return 1
    ...
    def remove_depart(self, name):
        ...
        if name not in [x['name'] for x in self._departs]:
            msg = u'A department with name %s is not in the list' % name
            self.last_msg = msg
            return 1
        for idx in range(len(self._departs)):
    ...
    def _undecorate_list(self, lst):
        ...
        new_lst = []
        for ll in lst:
            ll = self._udecorate_item(ll)
            if ll:
                new_lst.append(ll)
        return new_lst

    def sync_departments_order(self, lst):
        msg = 'It seems lst is not departments'
        assert len(lst) == len(self._departs) + 1, msg
...
class ShoppingList:
    ...
    def __init__(self):
        ...
        # WARNING: don't use pencil because the button starts its own menu
        # right after dialog open
        #
        # pencil key
        # self.products_list.ui_list.bind(0xf80b, self.at_add_product)
        ...
        appuifw.app.menu = [
                            ...
                            (ShoppingList.BY_DEPARTMENTS, self.at_mode),
    ...
    def at_mode(self):
        if self.product_mode:
            ...
            self.products_list.set_list([Products.ALL_DEPARTMENTS, ] +
                                        all_deps)
            self.product_mode = False
        else:
            # 'all goods' selected or return from home
            lst = self.products_list.lst
            if len(lst) and self.products.is_departments_in_list(lst):
                self.products.sync_departments_order(lst)
    ...
    def _load_products_to_the_list(self):
        ...
        self.products_list = Listbox(self.products.get_checked(),
                                     self.products_list_handler)
    ...
    def products_list_handler(self):
        ...
        if self.product_mode:
            ...
            self.products_list.set_list(lst
                                        if len(lst) else [EMPTY_LIST_MARK, ])
            appuifw.app.menu[1] = (ShoppingList.BY_DEPARTMENTS, self.at_mode)
    ...
    def at_home(self):
        ...
        if len(unchecked) == 0:
            ...
            idxs = appuifw.multi_selection_list(unchecked, style='checkbox',
                                                search_field=1)
            for idx in idxs:
    ...
    def at_add_product(self):
        ...
        if idx is not None:
            ...
            # : used to separate product name
            # and department in 'by depart' mode
            if prod_name and u':' not in prod_name:
                if not prod_name.lower() in self.products.get_all_products():
                    ...
                    if self.products.add_product(prod_name.lower(),
                                                 depart_name):
                        msg = u"Cannot add the item.\n%s"
                        appuifw.note(msg % self.products.last_msg, "error")
                    else:
                        self.products_list.add_item(prod_name.lower())
                else:
                    msg = u'%s has been at the list.'
                    appuifw.note(msg % prod_name.lower(), "error")
            else:
    ...
    def at_remove_depart(self):
        ...
        if idx is not None:
            depart_name = lst[idx]
            remove = appuifw.query(u"Remove %s?" % depart_name, "query") == 1
            if remove and self.products.remove_depart(depart_name):
                appuifw.note(u"%s" % self.products.last_msg, "error")
        appuifw.app.title = ShoppingList.TITLE
